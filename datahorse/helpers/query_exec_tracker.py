...
from datahorse.__version__ import __version__
from datahorse.connectors import BaseConnector
from datahorse.helpers.encoder import CustomEncoder
from datahorse.pipelines.chat.chat_pipeline_input import (
    ChatPipelineInput,
)
...
class QueryExecTracker:
    ...
    def publish(self) -> None:
        ...
        if self._server_config is None:
            ...
            api_key = os.environ.get("DATAHORSE_API_KEY") or None
        else:
            server_url = self._server_config.get(
                "server_url", os.environ.get("PANDASAI_API_URL", "https://api.domer.ai")
            )
            api_key = self._server_config.get(
                "api_key", os.environ.get("DATAHORSE_API_KEY")
            )
